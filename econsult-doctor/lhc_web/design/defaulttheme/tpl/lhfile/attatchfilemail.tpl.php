...
<?php foreach ($items as $file) : ?>
    <tr>         
        ...
        <td nowrap><a id="embed-button-<?php echo $file->id?>" onclick="lhinst.sendLinkToMail('[file=<?php echo $file->id,'_',$file->security_hash?>] <?php echo htmlspecialchars($file->upload_name,ENT_QUOTES)?>','<?php echo $file->id?>')" href="#" class="csfr-required btn btn-secondary btn-xs"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('file/list','Embed BB code');?></a></td>
    </tr>
<?php endforeach; ?>
</table>
